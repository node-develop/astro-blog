# 01. Что такое Claude Code: harness, agent loop и ваше место в нём
...
**Ключевая мысль:** модель сама по себе ничего не делает на вашей машине. Все действия — чтение файлов, запуск команд, вызовы MCP — это **tool calls**, которые исполняет harness. Модель только решает, *что* вызвать.
...
См. подробно в [02-context-and-cache.md](./02-context-and-cache.md).
...
🧪 **Agent Teams** — экспериментальная фича, требует `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. См. [10-agent-teams.md](./10-agent-teams.md).
...
В [12-travel-agent-blueprint.md](./12-travel-agent-blueprint.md) собирается финальная структура репозитория с всеми артефактами.
...
|---------|-----------|-------|
| `/context` | Визуализирует текущее заполнение окна | [02](./02-context-and-cache.md) |
| `/compact [hint]` | Сжимает историю, освобождая место | [02](./02-context-and-cache.md) |
| `/clear` | Полный сброс сессии (пере-стартует, кэш теряется) | [02](./02-context-and-cache.md) |
| `/model [name]` | Сменить модель в текущей сессии | [02](./02-context-and-cache.md), [10](./11-models-and-pricing.md) |
| `/agents` | Менеджер subagent'ов | [09](./09-subagents.md) |
| `/plugin install <ref>` | Установить плагин из marketplace | [07](./07-plugins.md) |
| `/mcp` | Список подключённых MCP-серверов | [06](./06-mcp.md) |
| `/permissions` | Текущие правила allow/deny | [05](./05-hooks.md) |
