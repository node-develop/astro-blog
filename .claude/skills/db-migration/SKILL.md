...
## Известный пробел: `drizzle/meta/0002_snapshot.json` отсутствует

`drizzle/meta/_journal.json` содержит запись `idx: 2` (`0002_revisions_prune_trigger` — ручная SQL-миграция с триггером), но снапшота `0002_snapshot.json` нет: есть `0000`, `0001`, `0003`, `0004`, `0005`. drizzle-kit при `pnpm db:generate` диффует схему против последнего снапшота, поэтому результат может содержать ложные изменения (например, повторное создание объектов, которых нет в цепочке снапшотов).

Что делать, если `pnpm db:generate` выдал неожиданный diff:

1. Не коммитить сгенерированный SQL вслепую и не «чинить» руками, копируя соседний снапшот под именем `0002_snapshot.json`.
2. На машине с доступом к БД (локальный `docker compose up -d postgres` + применённые миграции): `pnpm exec drizzle-kit check` — покажет расхождения между снапшотами и журналом.
3. Затем `pnpm db:generate` заново и ревью SQL: должны остаться только изменения, которые ты внёс в `schema.ts`.
4. Если drizzle-kit продолжает генерировать мусор — это отдельная задача на регенерацию снапшотов (`drizzle-kit up` / пересборка meta), обсуждается с пользователем, а не решается внутри фичи. Fail loud.

Раздел «Деплой» в `CLAUDE.md` дублирует это предупреждение.
