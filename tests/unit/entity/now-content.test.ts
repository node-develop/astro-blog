...
describe.each(["ru", "en"] as const)("now content (%s)", (locale) => {
  const prefix = locale === "en" ? "/en" : "";
  const md = readFileSync(join(process.cwd(), `src/content/site${prefix}/now.md`), "utf8");
  ...
  it("links to related pages in the same locale", () => {
    for (const route of ["about", "contact", "courses/claude-code-guide"]) {
