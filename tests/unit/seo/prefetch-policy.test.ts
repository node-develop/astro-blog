...
describe("prefetch policy", () => {
  it("does not prefetch on viewport entry", async () => {
    const { default: config } = await import("../../../astro.config");
    expect(config.prefetch).toBeDefined();
    const strategy =
      typeof config.prefetch === "object" ? config.prefetch.defaultStrategy : undefined;
    expect(strategy).toBeDefined();
    expect(strategy).not.toBe("viewport");
  });

  const built = existsSync(CLIENT);
  const skipReason = built ? null : "dist/client not found — run `pnpm build` first";

  it.skipIf(skipReason !== null)("opts every footer feed link out of prefetching", () => {
