...
const NOT_FOUND_PAGE = join(CLIENT, "404.html");
const NOT_FOUND_SOURCE = join(process.cwd(), "src", "pages", "404.astro");
...
describe("hreflang markup on the built site", () => {
  ...
  const skipReason = build
    ? null
    : "dist/client + sitemap-{ru,en}.xml not found — run `pnpm build` first";

  it.skipIf(skipReason !== null)("declares only language codes the site actually serves", () => {
  ...
  it.skipIf(skipReason !== null)("uses the same code vocabulary as the built sitemaps", () => {
  ...
  it.skipIf(skipReason !== null)("keeps the cluster off pages closed to indexing", () => {
  ...
  it.skipIf(skipReason !== null)("keeps the cluster off the not-found page", () => {
    if (!existsSync(NOT_FOUND_PAGE)) {
      // Nothing static to inspect because the route is server-rendered; the
      // production smoke test covers the response it returns at runtime. Pin
      // the reason, so a build that starts emitting a static 404 falls into
      // the branch below instead of quietly skipping the rule.
      expect(readFileSync(NOT_FOUND_SOURCE, "utf8")).toContain("export const prerender = false");
      return;
    }
    expect(markupCodes(readFileSync(NOT_FOUND_PAGE, "utf8"))).toEqual([]);
  });
});
