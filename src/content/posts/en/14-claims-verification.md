...
| --- | -------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Claude Code is a harness around an LLM, not the model itself                     | ✅     | See [01.](./01-introduction.md), [08.](./08-tool-calls-and-loop.md). The harness manages context, tools, hooks, permissions.                                                                                     |
...
| 3   | Context is limited to 200k tokens, but can be expanded to 1M                     | 🟡     | True, but 1M is only available via alias `opus[1m]` / `sonnet[1m]` and only on Max/Team/Enterprise. See [11.6.](./11-models-and-pricing.md#116-1m-контекст-когда-оправдан)                                       |
...
| 5   | Prompt cache gives huge savings                                                  | ✅     | Cache read = **0.1×** input price (10× cheaper). On a 30-turn session, savings are **65-80%**. See [02.](./02-context-and-cache.md), [11.5.](./11-models-and-pricing.md#115-расчёт-типичной-сессии-travel-agent) |
| 6   | Cache TTL is 5 minutes by default, can be extended to 1 hour                     | ✅     | 5min cache write = 1.25× input, 1h cache write = 2× input.                                                                                                                                                       |
| 7   | There's an "Advisor mode" for planning on Opus                                   | ❌     | The correct name is **`opusplan`**. See [02.6.](./02-context-and-cache.md), [11.4.2.](./11-models-and-pricing.md). After ExitPlanMode, automatically switches to Sonnet.                                         |
...
| 10  | CLAUDE.md has a hierarchy: project, user, local                                  | 🟡     | Actually **5 levels**: managed (enterprise) > user (`~/.claude/CLAUDE.md`) > project (`./CLAUDE.md`) > project local (`./CLAUDE.local.md`, deprecated) > subdirectory CLAUDE.md. See [03.](./03-claude-md.md).   |
| 11  | CLAUDE.md supports @-imports                                                     | ✅     | Recursively up to 5 levels. Use `@./path/to/file.md`.                                                                                                                                                            |
| 12  | Skill is just a markdown file                                                    | ❌     | **Skill is a directory** with `SKILL.md` + optionally `scripts/`, `references/`, `templates/`. See [04.](./04-skills.md).                                                                                        |
| 13  | Hooks are shell commands on lifecycle events                                     | ✅     | Confirmed. But there are **more than 6** events (as in the thread) — actually **28+**. See [05.1.](./05-hooks.md#51-полный-список-событий)                                                                       |
...
| 15  | MCP servers support stdio, SSE, HTTP                                             | ✅     | All three transports. See [06.2.](./06-mcp.md). In practice, most servers use stdio.                                                                                                                             |
| 16  | Plugins allow packaging MCP + skills + hooks                                     | ✅     | Manifest `.claude-plugin/plugin.json`, can be published via marketplaces. See [07.](./07-plugins.md).                                                                                                            |
| 17  | Subagents have isolated context and return only summary                          | ✅     | Each subagent is a separate API call with its own prefix. Final text is returned to main context. See [09.](./09-subagents.md).                                                                                  |
...
| 20  | Agent Teams is a full-featured capability for parallel work of multiple Claude   | 🟡🧪   | Real feature since v2.1.32. But **experimental**, requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. See [10.](./10-agent-teams.md).                                                                             |
...
| 28  | Permissions have 3 levels: allow / ask / deny                                    | ✅     | `deny` is final, can't be bypassed even in `bypassPermissions`. See [08.5.](./08-tool-calls-and-loop.md#85-permissions-как-claude-code-решает-спросить-или-нет)                                                  |
...
| 30  | "With new models, you need to review CLAUDE.md and skills"                       | 🟡     | Not a docs quote, but sound practice. See [11.8.](./11-models-and-pricing.md#118-стоит-ли-пересматривать-claudemd-и-skills-с-новыми-моделями).                                                                   |
