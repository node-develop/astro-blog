---
title: Local coding agent on DeepSeek V4 Flash running on MacBook
description: >-
  The creator of Redis spent two weeks building an inference engine for just one model — DeepSeek V4 Flash. 1M context,
  26 t/s on M3 Max, KV-cache on disk. How to run it and connect it to Claude Code.
...
tags:
  ...
  - coding-agents
draft: false
coverAlt: ds4 — local inference engine for DeepSeek V4 Flash on Apple Metal
lang: en
sourceHash: 381e753eccc07206ebd7f1879765bb4775c526d2bcc89d62169c8f5bfd481ec2
...
- question: "How is this better than OpenAI/Anthropic API?"
    answer: "Not better — different. Cloud API is always faster, more reliable, and the models on top are smarter (current frontier is Sonnet 4.6, Opus 4.6, GPT 5.5). ds4 on DeepSeek V4 Flash is 'quasi-frontier' with three advantages: zero rubles per token, full data control, offline operation. Good for private projects, agent experiments, and working on a plane. Not suitable if you need maximum accuracy or work with context that won't fit on your Mac."
pubDate: 2026-05-09
tags:
- ai
- local-inference
...
On April 24, 2026, DeepSeek released the V4 series. V4 Flash is an efficiency model: 284 billion parameters total, 13 billion active (MoE), 1 million token context. Before, models of this size only lived in the cloud.

Antirez looked at this and made a bet that universal runners can't make. He forked `llama.cpp`, spent two weeks inside it, understood V4 Flash geometry, **threw out everything unnecessary** and wrote from scratch an engine in 4 files: `ds4.c` (~ inference), `ds4_metal.m` (Metal kernels), `ds4_server.c` (HTTP server), `ds4_cli.c` (REPL). On the outside, all of this speaks two protocols simultaneously: OpenAI Chat Completions (`/v1/chat/completions`) and Anthropic Messages (`/v1/messages`). That is, it connects to any agent that knows one of them.
...
The main pain of stateless API protocols like Chat Completions: the client **sends the entire history every time**, and the server must prefill it from scratch. Claude Code, for example, sends ~25K tokens of system prompt at startup. On local hardware this is tens of seconds to first token.

Ds4 solves this head-on: after successful prefill, the session state (KV checkpoint) is serialized to a file, key is SHA1 of token IDs. When the next request comes with the same prefix, the server takes the checkpoint from disk and skips prefill. From the README:
...
No CUDA, no CPU fallback for prod (CPU path exists only for correctness checks and currently crashes at the kernel level on macOS due to a VM bug — antirez writes about this honestly). No attempt to make a 'universal runner'. Only Apple Silicon, only this one model, until a new version of V4 Flash or a much better model of the same class appears.

The price is a narrow bet. The benefit is you don't need to maintain a matrix `(model × hardware × quant)`, and you can optimize Metal kernels for the exact geometry of layers in this specific model.

---

## 3. What I'll need: hardware, model, an hour of time

I plan to deploy this on **MacBook Pro M3 Max, 128 GB** (minimum viable configuration per README). I don't have it yet, and in this section — an honest plan of what I'll do when the hardware arrives; the numbers are taken from antirez's benchmarks, but I want to double-check them on my instance.
...
Build is a regular `make`, no CMake, no pkg-config. This is intentional: the project has no dependencies outside Apple SDK.
...
- `--ctx 100000` — context window of 100K tokens. Full 1M context eats ~26 GB just for the indexer; on a 128 GB Mac where 81 GB is already taken by the model, this leaves no room for KV cache. 100–300K is a reasonable compromise.
- `--kv-disk-dir /tmp/ds4-kv` — directory for disk KV cache. I'd move it to a fast SSD (external or built-in — both ok).
- `--kv-disk-space-mb 8192` — cache size limit. 8 GB for one or two active projects is enough; for larger sessions — increase it.
...
`cost: 0` — this isn't marketing, it's true. Each request costs electricity and SSD wear, not tokens.
...
If this bet works (and benchmarks say it does), the future of local inference is not 'yet another abstraction on top of abstraction', but **'each important model gets its own ds4-like project'**. When V4.1 or V5 comes out, someone from the community makes a new engine, new GGUF, new tests, and in two weeks users already have a working local setup. Old engines retire with old models.
