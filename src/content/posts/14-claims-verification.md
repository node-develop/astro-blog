...
# 14. Проверка утверждений из исходного треда
...
| --- | ------------------------------------------------------------------------------ | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Claude Code — это harness вокруг LLM, а не сама модель                         | ✅     | См. [01.](./01-introduction.md), [08.](./08-tool-calls-and-loop.md). Harness управляет контекстом, tools, hooks, разрешениями.                                                                                       |
...
| 3   | Контекст ограничен 200k токенов, но можно расширить до 1M                      | 🟡     | Верно, но 1M доступен только через alias `opus[1m]` / `sonnet[1m]` и только на Max/Team/Enterprise. См. [11.6.](./11-models-and-pricing.md#116-1m-контекст-когда-оправдан)                                           |
...
| 5   | Prompt cache даёт огромную экономию                                            | ✅     | Cache read = **0.1×** input price (10× дешевле). На 30-turn сессии экономия **65-80%**. См. [02.](./02-context-and-cache.md), [11.5.](./11-models-and-pricing.md#115-расчёт-типичной-сессии-travel-agent)            |
| 6   | TTL кэша по умолчанию — 5 минут, можно расширить до 1 часа                     | ✅     | 5min cache write = 1.25× input, 1h cache write = 2× input.                                                                                                                                                           |
| 7   | Есть «Advisor mode» для планирования на Opus                                   | ❌     | Правильное имя — **`opusplan`**. См. [02.6.](./02-context-and-cache.md), [11.4.2.](./11-models-and-pricing.md). После ExitPlanMode автоматически переключается на Sonnet.                                            |
...
| 10  | CLAUDE.md имеет иерархию: project, user, local                                 | 🟡     | На самом деле **5 уровней**: managed (enterprise) > user (`~/.claude/CLAUDE.md`) > project (`./CLAUDE.md`) > project local (`./CLAUDE.local.md`, deprecated) > subdirectory CLAUDE.md. См. [03.](./03-claude-md.md). |
| 11  | CLAUDE.md поддерживает @-imports                                               | ✅     | Рекурсивно до 5 уровней. Используйте `@./path/to/file.md`.                                                                                                                                                           |
| 12  | Skill — это просто markdown-файл                                               | ❌     | **Skill — это директория** с `SKILL.md` + опционально `scripts/`, `references/`, `templates/`. См. [04.](./04-skills.md).                                                                                            |
| 13  | Hooks — это shell-команды на события lifecycle                                 | ✅     | Подтверждено. Но событий **больше 6** (как в треде) — реально **28+**. См. [05.1.](./05-hooks.md#51-полный-список-событий)                                                                                           |
...
| 15  | MCP-серверы поддерживают stdio, SSE, HTTP                                      | ✅     | Все три транспорта. См. [06.2.](./06-mcp.md). На практике большинство серверов используют stdio.                                                                                                                     |
| 16  | Plugins позволяют пакетировать MCP + skills + hooks                            | ✅     | Манифест `.claude-plugin/plugin.json`, можно публиковать через marketplaces. См. [07.](./07-plugins.md).                                                                                                             |
| 17  | Subagents имеют изолированный контекст и возвращают только summary             | ✅     | Каждый subagent — отдельный API-call с своим префиксом. Возвращается финальный текст в основной контекст. См. [09.](./09-subagents.md).                                                                              |
...
| 20  | Agent Teams — полноценная фича для параллельной работы нескольких Claude       | 🟡🧪   | Реальная фича с v2.1.32. Но **экспериментальная**, требует `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. См. [10.](./10-agent-teams.md).                                                                                 |
...
| 28  | Permissions имеют 3 уровня: allow / ask / deny                                 | ✅     | `deny` — финальный, нельзя обойти даже в `bypassPermissions`. См. [08.5.](./08-tool-calls-and-loop.md#85-permissions-как-claude-code-решает-спросить-или-нет)                                                        |
...
| 30  | «С новыми моделями надо пересматривать CLAUDE.md и skills»                     | 🟡     | Не цитата из docs, а здравая практика. См. [11.8.](./11-models-and-pricing.md#118-стоит-ли-пересматривать-claudemd-и-skills-с-новыми-моделями).                                                                      |
...
В треде было ~6 событий. Реально на v2.1.89 их **28+**. Полный список в [05.1.](./05-hooks.md#51-полный-список-событий). Особенно полезные, которых часто не знают:
...
- `/anthropics/anthropic-sdk-typescript` — для SDK-кода в [06.](./06-mcp.md) и [12.](./12-travel-agent-blueprint.md).
...
**Назад →** [README (оглавление)](./README.md)
