---
title: "12. Travel Agent from Scratch: blueprint"
blurb: "Bringing it all together. One real project. One monorepo. Specific files, configs, structure. From here on, you copy-paste and adapt."
pubDate: 2026-04-23
order: 12
locale: en
---
...
**Travel Agent** — an AI service that helps plan a trip. Architecturally:
...
flowchart TB
  ...
  subgraph mcps["MCP servers (locally or remotely)"]
    mcpf["mcp-flights<br/>Amadeus"]
    mcph["mcp-hotels<br/>Duffel"]
    mcpw["mcp-weather<br/>OpenMeteo"]
    mcpd["mcp-docs<br/>our wiki"]
  end

  api -->|MCP stdio/HTTP| mcps

  subgraph claude["Claude Code (for development)"]
    cc["Developer CLI"]
  end
...
In **development** we use Claude Code (CLI) to write code, tests, migrations. At **runtime** our backend (apps/api) talks to the Anthropic API directly via SDK, reusing the same MCP servers.
...
## 12.3. Root CLAUDE.md (ready template)
...
## 12.4. .claude/settings.json (ready)
...
4. Define tools in `src/server.ts`:
   - `{name}_search` — search.
   - `{name}_get_details` — details.
   - If needed — `{name}_book`, `{name}_cancel`.
...
## 12.9. A day in the life of a developer

Scenario: "add a route sharing feature with short links".
...
sequenceDiagram
  ...
  U->>CC: /model opusplan
  U->>CC: "Design a route sharing feature with short links"
  CC->>CC: Plan mode (Opus): architecture, risks
  CC-->>U: plan + critique
  U->>CC: ExitPlanMode → implement
  CC->>CC: switch to Sonnet
  CC->>CC: skill backend-add-migration activated
  ...
  CR-->>CC: report: 1 critical (missing rate-limit), 3 nits
  CC->>CC: Edit to fix critical
...
## 12.10. What to commit to the repo, what not to
...
## 12.11. Bootstrap a new developer
...
And all the infrastructure (skills, agents, hooks, default MCP config) appears with one command.
